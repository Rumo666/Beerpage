...
export default function BierDetailPage() {
    ...
    const handleSave = async () => {
        ...
        const res = await fetch(`/api/bier/${id}`, {
            ...
            body: JSON.stringify({
                sterne: editSterne,
                notizen: editNotizen,
                geschmack: editGeschmack,
                mundgefuehl: editMund,
                kohlensaeure: editCo2,
                farbe: editFarbe,
                preis: editPreis,
                nochmal: editNochmal,
            }),
    ...
    return (
        <div style={{ minHeight: "100vh", background: C.bg, color: C.tx, fontFamily: "system-ui, sans-serif", paddingBottom: 40 }}>
            ...
            <div style={{ background: C.card, borderBottom: `1px solid ${C.bd}`, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, position: "sticky" as const, top: 0, zIndex: 50 }}>
                ...
                {canEdit && (
                    <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setEditMode(true)} style={{ padding: "6px 12px", borderRadius: 8, border: `1px solid ${C.bd}`, background: "transparent", color: C.txD, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                            ✎ Bearbeiten
                        </button>
                        <button onClick={() => setShowDeleteConfirm(true)} style={{ padding: "6px 12px", borderRadius: 8, border: `1px solid ${C.danger}40`, background: "transparent", color: C.danger, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                            🗑 Löschen
                        </button>
                    </div>
                )}
            ...
            {/* Admin Badge */}
            {isAdmin && !canEdit && (
                <div style={{ margin: "12px 16px 0", padding: "8px 14px", borderRadius: 10, background: "rgba(200,150,62,0.1)", border: `1px solid ${C.ac}30`, fontSize: 12, color: C.ac }}>
                    👑 Du siehst diese Bewertung als Admin
                </div>
            )}
                ...
                <div style={{ background: C.card, borderRadius: 14, border: `1px solid ${C.bd}`, overflow: "hidden", marginBottom: 12 }}>
                    ...
                    <div style={{ padding: 16 }}>
                        ...
                        <button onClick={handleProst} style={{
                            width: "100%", padding: "10px", borderRadius: 10, border: `1.5px solid ${geprosted ? C.ac : C.bd}`,
                    ...
                    {b.geschmack?.length > 0 && (
                        <div style={{ marginBottom: 8 }}>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 5 }}>
                                {b.geschmack.map((g: string) => <span key={g} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 12, background: C.acGlow, color: C.ac, fontWeight: 600, border: `1px solid ${C.ac}30` }}>{g}</span>)}
                            </div>
                        </div>
                    )}
                        ...
                        <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.bd}`, padding: 20 }}>
                            ...
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <div style={{ display: "flex", gap: 4 }}>
                                    {[1,2,3,4,5].map(s => (
                                        <span key={s} onClick={() => setEditSterne(s)} style={{ fontSize: 34, color: s <= editSterne ? C.ac : C.bd, cursor: "pointer", userSelect: "none" as const }}>★</span>
                                    ))}
                                </div>
                            </div>
                            ...
                            {/* Geschmack */}
                            <div style={{ marginBottom: 16 }}>
                                <div style={{ fontSize: 11, color: C.txD, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 8 }}>Geschmack</div>
                                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                                    {GESCHMACK_OPT.map(g => <Chip key={g} label={g} active={editGeschmack.includes(g)} onClick={() => toggle(editGeschmack, setEditGeschmack, g)} />)}
                                </div>
                            </div>

                            {/* Mundgefühl */}
                            <div style={{ marginBottom: 16 }}>
                                <div style={{ fontSize: 11, color: C.txD, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 8 }}>Mundgefühl</div>
                                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                                    {MUND_OPT.map(m => <Chip key={m} label={m} active={editMund === m} onClick={() => setEditMund(editMund === m ? "" : m)} />)}
                                </div>
                            </div>

                            {/* Kohlensäure */}
                            <div style={{ marginBottom: 16 }}>
                                <div style={{ fontSize: 11, color: C.txD, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 8 }}>Kohlensäure</div>
                                <div style={{ display: "flex", gap: 6 }}>
                                    {CO2_OPT.map(k => <Chip key={k} label={k} active={editCo2 === k} onClick={() => setEditCo2(editCo2 === k ? "" : k)} />)}
                                </div>
                            </div>

                            {/* Farbe */}
                            <div style={{ marginBottom: 16 }}>
                                <div style={{ fontSize: 11, color: C.txD, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 8 }}>Farbe</div>
                                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                                    {FARB_OPT.map(f => <Chip key={f} label={f} active={editFarbe === f} onClick={() => setEditFarbe(editFarbe === f ? "" : f)} />)}
                                </div>
                            </div>

                            {/* Preis */}
                            <div style={{ marginBottom: 16 }}>
                                <div style={{ fontSize: 11, color: C.txD, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 8 }}>Preis-Leistung</div>
                                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                                    {PREIS_OPT.map(p => <Chip key={p} label={p} active={editPreis === p} onClick={() => setEditPreis(editPreis === p ? "" : p)} />)}
                                </div>
                            </div>
                            ...
                            <div style={{ display: "flex", gap: 10 }}>
                                <button onClick={handleSave} disabled={saving} style={{ flex: 1, padding: 14, background: `linear-gradient(135deg, ${C.ac}, ${C.acDim})`, color: C.bg, border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
                                    {saving ? "Speichern…" : "💾 Speichern"}
                                </button>
                                <button onClick={() => setEditMode(false)} style={{ padding: "14px 20px", background: "transparent", color: C.txD, border: `1px solid ${C.bd}`, borderRadius: 10, cursor: "pointer", fontSize: 14 }}>
                                    Abbrechen
                                </button>
                            </div>
                        </div>
            ...
            {showDeleteConfirm && (
                <div style={{ position: "fixed" as const, inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
                    <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.danger}40`, padding: 24, maxWidth: 360, width: "100%" }}>
                        ...
                        <div style={{ display: "flex", gap: 10 }}>
                            <button onClick={handleDelete} disabled={deleting} style={{ flex: 1, padding: 12, background: C.danger, color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
                                {deleting ? "Löschen…" : "Ja, löschen"}
                            </button>
                            <button onClick={() => setShowDeleteConfirm(false)} style={{ flex: 1, padding: 12, background: "transparent", color: C.txD, border: `1px solid ${C.bd}`, borderRadius: 10, cursor: "pointer", fontSize: 14 }}>
                                Abbrechen
                            </button>
                        </div>
                    </div>
